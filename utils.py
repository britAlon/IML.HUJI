...
def decision_surface(predict, xrange, yrange, num_classifiers=1, density=120, dotted=False, colorscale=custom, showscale=True):
    xrange, yrange = np.linspace(*xrange, density), np.linspace(*yrange, density)
    xx, yy = np.meshgrid(xrange, yrange)
    if num_classifiers > 1:
        pred = predict(np.c_[xx.ravel(), yy.ravel()], num_classifiers)
    else:
